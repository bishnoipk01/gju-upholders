...
const getQuestion = async (id) => {
  const res = await fetch(
    `https://${process.env.VERCEL_URL}/api/questions/${id}`
  );
...
const getAnswers = async (id) => {
  const res = await fetch(
    `https://${process.env.VERCEL_URL}/api/questions/${id}/answers`
  );
  const data = await res.json();
  return data.data;
};
...
export default function QuestionById({ params }) {
  ...
  async function postAnswer(e) {
    ...
    const res = await fetch(
      `https://${process.env.VERCEL_URL}/api/questions/${params.id}`,
      {
        method: 'post',
